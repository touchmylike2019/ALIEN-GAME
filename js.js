class Game {
    ...
    showGrade(score) {
        if(score > 30) return "คุณมันไม่ใช่คนเเล้ว!";
        else if(score > 25) return "คุณจะเก่งเกินไปเเล้ว!";
        else if(score > 20) return "สุดยอด!";
        else if(score > 15) return "เยี่ยมไปเลย";
        else if(score > 10) return "ไปให้ไกลกว่านี้อีก!";
        else if(score > 5) return "เเค่นี้พอเเล้วหรอ?";
        else return "น่าสงสาร...";
    }
...
class Animation {
    ...
    static sceneAnimation() {
        ...
        var wave1 = TweenMax.to('.wave1', speed*8, {backgroundPositionX: '-=1760px', repeat: -1, ease: Power0.easeNone});
        var wave2 = TweenMax.to('.wave2', speed*8, {backgroundPositionX: '-=1760px', repeat: -1, ease: Power0.easeNone});
